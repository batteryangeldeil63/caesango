/* global app, lstCarSolicitacao, lstTipoAtendimento, lstTipoConsulta, lstIndicAcidente, lstCodCbo, lstConPrestSol, lstUfConselho, token, lstProfissionais, secretario, webservice, objVO0306 */

app.controller("ctrSpSadt", function($scope, $http) {
    ...
    $scope.buscaListTab(572, 2, 23); // lstCarSolicitacao
    ...
    */
    ...
    $scope.buscarProcedimentoTussCodigo = function() {
        ...
        $scope.alertabloqueio = false;
        if (
            $scope.objVO0482.codtabela &&
            $scope.objVO0482.codprocedimento &&
            $scope.objVO0482.codprocedimento.length >= 8
        ) {
            const body = {
                coddao: 526,
            ...
            $http({
                ...
                function successCallback(response) {
                    if (response.data.conteudo.length > 0) {
                        $scope.selecionarProcedimento(
                            response.data.conteudo[0].codprocedimento,
                            response.data.conteudo[0].descricao,
                            response.data.conteudo[0].procbloqueado
                        );
                    } else {
            ...
            if ($scope.consultaTuss != null) {
                if ($scope.consultaTuss.codigo_interno != $scope.objVO0482.codtabela) {
                    for (i = 0; i < $scope.procTuss.length; i++) {
                        if (
                            $scope.procTuss[i].codigo_interno == $scope.objVO0482.codtabela
                        ) {
    ...
    $scope.adicionarProcedimento = function() {
        ...
        for (i = 0; i < $scope.objVO0481.procedimentos.length; i++) {
            if (
                $scope.objVO0481.procedimentos[i].codtabela ===
                $scope.objVO0482.codtabela &&
